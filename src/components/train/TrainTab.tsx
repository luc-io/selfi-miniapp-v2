...
import { Loader2, Upload, X, Edit2, InfoIcon } from 'lucide-react';
import { 
  startTraining, 
  uploadTrainingFiles
} from '@/lib/api';
          ...
          <div className="space-y-2">
            <div className="flex items-center space-x-2">
              <label className="block text-sm font-medium text-gray-700" htmlFor="triggerWord">
                {state.isStyle ? 'Style Name' : 'Subject Name'}
              </label>
              <div className="group relative">
                ...
                <div className="absolute left-0 bottom-6 w-64 p-2 bg-gray-800 text-white text-xs rounded opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                  {state.isStyle 
                    ? "Name your style (e.g., 'anime_style', 'watercolor'). This will be used to activate the style in image generation."
                    : "Enter a word to identify your subject (e.g., 'my_cat', 'my_house'). This will be used to reference your subject in prompts."}
                </div>
              </div>
            </div>
            <input
              ...
              placeholder={state.isStyle 
                ? "Enter a name for your style (e.g., anime_style)" 
                : "Enter a word to identify your subject (e.g., my_cat)"}
              className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
              required
            />
            <p className="text-xs text-gray-500">
              {state.isStyle 
                ? "This name will be used to activate your style during image generation" 
                : "This word will be used to reference your subject in generation prompts"}
            </p>
          </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-sm text-gray-500">Use AI to focus on the main subject</p>
              </div>
              <Switch
